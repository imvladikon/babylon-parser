...
public class DictEntry {
	...
	public DictEntry(BglBlock block, Charset srcEncoding, Charset dstEncoding) {
		this.block = block;
	...
	private void parseDefinitionPart(byte[] defdata) {
	...
	private void parseDefinitionMetaInfo(byte[] data) {
		int len = data.length;
		int pos = 0;
	...
	private int readNum(byte[] data, int offset, int bytes) {
		int val = 0;
	...
	private static String readString(byte[] data, int pos, int len, Charset charset) {
		try {
			return new String(data, pos, Math.min(len, data.length - pos), charset);
